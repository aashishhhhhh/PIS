...
class AddressController extends Controller
{
    ...
    {
        $id = $request->id;
        // $data = Municipality::query()->selectRaw("id,did,CONCAT(name, ' ', type) as name, CONCAT(nep_name, ' ', nep_type) as nep_name, total_wards")
        //     ->where('did', $id)->get();

        $data = Municipality::query()->where('did', $id)->get();
        return response()->json($data);
    }
}
